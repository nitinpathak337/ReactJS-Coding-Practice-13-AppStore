...
class AppStore extends Component {
  state = {category: 'SOCIAL', inputValue: ''}

  changeCategory = catgry => {
    this.setState({category: catgry})
  }
  ...
  render() {
    ...
    const filteredSearchAppList = filteredCategoryAppList.filter(eachItem =>
        ...
        <ul className="list-container">
          {tabsList.map(eachItem => (
            <TabItem
              ...
              isActive={category === eachItem.tabId}
            />
          ))}
        </ul>
        <ul className="app-list-container">
          {filteredSearchAppList.map(eachItem => (
